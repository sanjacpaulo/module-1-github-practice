...
export default function BookingScreen() {
  ...
  useEffect(() => {
    async function restoreData() {
      try {
        const [restoredHotels, restoredCity] = await Promise.all([
          loadHotels(),
          loadSelectedCity(),
        ]);
        ...
        setStorageError('Failed to load saved data.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  async function toggleSavedHotel(hotel) {
    ...
    try {
      const isAlreadySaved = savedHotels.some((item) => item.id === hotel.id);
      const updatedHotels = isAlreadySaved
        ? savedHotels.filter((item) => item.id !== hotel.id)
        : [...savedHotels, hotel];
